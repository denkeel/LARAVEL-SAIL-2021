...
<form action="{{ route('admin/categories/store') }}" method="POST" class="flex w-min shadow-sm rounded-3xl mb-5 py-3 px-4">
    ...
    <div class="flex flex-row justify-center items-center">
        <input name="category" type="text" class="p-1.5 px-2 mr-3 focus:ring-blue-200 border-blue-300 border-2 focus:border-blue-500 focus:ring-4 outline-none focus:outline-none rounded-xl">
    </div>
...
<div @mousedown.away="isShowing = false;" x-data="{ isShowing: false }" class="flex w-5/12 bg-white shadow-sm justify-between rounded-3xl mb-3 py-1.5">
    <div class="flex flex-row justify-center items-center ml-3">
        ...
        <div $refs.editInput.value = $refs.editP.innerHTML" x-show="isShowing" action="{{ route('admin/categories/update', ['id' => $category['_id']]) }}" method="POST" class="flex ml-1.5">
            @csrf
            {{ method_field('PUT') }}
            <div class="flex flex-row justify-center items-center">
                ...
                <input x-ref="editInput" name="category" value="{{ $category['name'] }}" type="text" class="p-1.5 px-2 mr-3 focus:ring-gray-200 shadow-inner focus:border-gray-200 border-gray-200 border outline-none focus:outline-none rounded-xl">

            </div>
            <div class="flex flex-row">
                <button @click="$refs.editP.innerHTML = $refs.editInput.value;isShowing = false; edit('{{ route('admin/categories/update', ['id' => $category['_id']]) }}', $refs.editInput.value)" class="flex flex-col font-bold text-white justify-center outline-none focus:outline-none active:bg-green-700 bg-green-500 rounded-xl items-center p-2">OK</button>
            </div>
        </div> {{----}}
    </div>
    <div class="flex flex-row">
        ...
        <a class="h-10 w-10 flex flex-col justify-center hover:bg-red-500 rounded-xl items-center flex-grow-0 mr-3 text-red-600 hover:text-white" href="#" onclick="remove(event, '{{ route('admin/categories/destroy', ['id' => $category['_id']]) }}')">
