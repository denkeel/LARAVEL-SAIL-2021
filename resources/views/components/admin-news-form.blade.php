...
<form class="flex flex-col max-w-3xl space-y-6" action="{{ $action }}" method="post">
    ...
    <div class="flex flex-col">
        ...
        <textarea class="mb-4 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent shadow-sm rounded-md p-3" name="content" cols="30" rows="10" id="editor">{{ $oldContent ?? ''}}</textarea>
    </div>
